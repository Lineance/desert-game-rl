...
class RolloutBuffer:
    ...
    def add(self, obs, action, reward, value, log_prob, done, hidden=None, valid_actions=None):
        """添加一个时间步的数据"""
    ...
    def compute_returns_and_advantages(self, last_value: float, 
                                       gamma: float = 0.99, 
                                       gae_lambda: float = 0.95) -> Tuple[List, List]:
        """
        使用GAE计算回报和优势
...
class PPOTrainer:
    ...
    def __init__(self, 
        ...
        self.optimizer = optim.Adam([
            ...
            {'params': agent.state_encoder.parameters(), 'lr': self.config.LR_ACTOR},
            {'params': agent.belief_encoder.parameters(), 'lr': self.config.LR_ACTOR},
        ])
    ...
    def update(self, rollout_buffer: RolloutBuffer, 
        ...
        values_tensor = torch.FloatTensor(rollout_buffer.values).to(self.device)  # BUG修复6: 用于价值裁剪
        ...
        actions = {
            ...
            'buy_water': torch.FloatTensor([a['buy_water'] for a in rollout_buffer.actions]).to(self.device),
            'buy_food': torch.FloatTensor([a['buy_food'] for a in rollout_buffer.actions]).to(self.device),
        }
        ...
        for epoch in range(num_updates):
            ...
            for i in range(len(obs_tensor)):
                ...
                new_log_prob_i, entropy_i = self.agent.actor.evaluate_actions(state_i, action_i, valid_i)
    ...
    def collect_rollout(self, env, max_steps: int = 1000, epsilon: float = 0.0) -> Tuple[RolloutBuffer, float, Dict]:
        ...
        while step < max_steps:
            ...
            buffer.add(obs, action_idx, reward, value, log_prob, done_flag, valid_actions=valid_actions)
