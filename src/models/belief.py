...
class WeatherBeliefModel:
    ...
    def update(self, weather_today: int):
        ...
        if self.use_dirichlet_prior and len(self.weather_history) >= 5:
            ...
            self.transition_matrix = (
                (1 - self.learning_rate) * self.transition_matrix +
                self.learning_rate * new_transition
            )
    ...
    def get_belief_vector(self) -> np.ndarray:
        ...
        return np.concatenate([
            self.current_belief.probs,
            [self.current_belief.confidence]
        ])
    
    def predict_weather_sequence(self, 
                                 current_weather: int,
