...
def run_episode(agent: HybridRNNAgent, env, seed: int = None, 
                deterministic: bool = True, verbose: bool = False) -> Dict:
    """
    运行一个完整的回合
    ...
    while not done and step < 200:
        ...
        record = {
            ...
            'belief': next_info['belief'].tolist() if hasattr(next_info['belief'], 'tolist') else next_info['belief'],
        ...
        if verbose:
            print(f"Day {record['day']}: Pos={record['position']}, "
...
def export_to_csv(result: Dict, filepath: str, level: int):
    ...
    with open(filepath, 'w', newline='', encoding='utf-8') as f:
        ...
        # 表头
        writer.writerow(['日期', '区域', '剩余资金(元)', '剩余水量(箱)', 
                        '剩余食物量(箱)', '天气', '操作'])
...
def analyze_strategy(result: Dict, env):
    ...
    # 统计购买
    buy_records = [(r['day'], r['action']['buy_water'], r['action']['buy_food']) 
                   for r in records 
...
def evaluate_multiple_runs(agent_path: str, level: int, num_runs: int = 100,
                          device: str = None):
    """
    多次运行评估，统计性能
    """
...
def generate_result_excel(agent_path: str, level: int, 
    ...
    if best_result is None:
        ...
        # 选择回报最高的
        best_result = max([run_episode(agent, env, seed=i, deterministic=True) 
                          for i in range(20)], key=lambda x: x['return'])
...
def main():
    ...
    parser.add_argument('--level', type=int, default=3, choices=[3, 4],
                       help='关卡（3或4）')
    parser.add_argument('--runs', type=int, default=1,
                       help='运行次数（1表示单轮详细输出，>1表示统计）')
    ...
    if args.runs == 1:
        ...
        result = run_episode(agent, env, seed=args.seed, 
                           deterministic=True, verbose=args.verbose)
