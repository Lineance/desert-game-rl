...
class DesertCrossingEnv:
    ...
    def reset(self, seed=None) -> Tuple[np.ndarray, Dict]:
        ...
        self.belief_model = WeatherBeliefModel(
            transition_prior=self.config.WEATHER_TRANSITION.copy(),
            learning_rate=0.1
        )
    ...
    def _compute_initial_resources(self) -> Tuple[int, int]:
        ...
        # 调整重量
        total_weight = (water * self.config.WATER_WEIGHT + 
                       food * self.config.FOOD_WEIGHT)
    ...
    def _compute_purchase_cost(self, water: int, food: int, at_start: bool) -> float:
        """计算购买成本"""
        if at_start:
            return (water * self.config.WATER_PRICE_BASE + 
                   food * self.config.FOOD_PRICE_BASE)
        else:
            return (water * self.config.WATER_PRICE_BASE * 2 + 
    ...
    def step(self, action: Dict[str, Any]) -> Tuple[np.ndarray, float, bool, bool, Dict]:
        ...
        if s.day == 0:
            ...
            if buy_water > 0 or buy_food > 0:
                if s.day == 0 and s.position == self.config.START and not s.has_purchased_at_start:
                    ...
                    if cost <= s.money:
                        new_weight = (buy_water * self.config.WATER_WEIGHT +
            ...
            if at_village:
                ...
                if cost <= s.money:
                    new_weight = ((s.water + buy_water) * self.config.WATER_WEIGHT +
                                 (s.food + buy_food) * self.config.FOOD_WEIGHT)
                    if new_weight <= self.config.WEIGHT_LIMIT:
                        s.water += buy_water
                        s.food += buy_food
                        s.money -= cost
        ...
        if s.position == self.config.END:
            ...
            terminated = True
            # 退回剩余资源
            refund = (s.water * self.config.WATER_PRICE_BASE * 0.5 + 
    ...
    def get_valid_actions(self) -> Dict[str, Any]:
        ...
        can_mine = (
            s.position in self.config.MINES and
            len(s.path_history) >= 2 and
            s.path_history[-1] == s.path_history[-2]
        )
        
        can_buy = (
            (s.day == 0 and s.position == self.config.START and not s.has_purchased_at_start) or
            (s.position in self.config.VILLAGES)
        )
